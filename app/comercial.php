...
function render_comercial_page() {
    ...
    if ($tab === 'procesos') {
        ...
        if ($selectedProcess) {
            ...
            echo '<div class="form-grid-2">';
            comercial_field_number('conversation_max_auto_turns', 'Respaldo (legacy)', $selectedProcess['conversation_max_auto_turns'] ?? 999);
            ...
            echo '</div>';
            echo '<div class="field-help" style="margin-top:-6px; margin-bottom:12px;">El bot ya no se limita por número de respuestas: decide seguir según el contexto de toda la conversación. Este valor queda solo como respaldo legacy. | Si el score de interés supera el umbral, se notifica al operador humano.</div>';
...
HTML;
    ...
    if ($tab === 'ajustes') {
        ...
        comercial_field_number('conversation_max_auto_turns', 'Respaldo turnos auto (legacy)', $settings['conversation_max_auto_turns'], '1');
        comercial_field_number('conversation_max_defers', 'Máx defer por hilo', $settings['conversation_max_defers'], '1');
        echo '</div>';
        echo '<div class="field-help" style="margin-top:-6px; margin-bottom:12px;">El bot ya no se limita por turnos: decide según el contexto de toda la conversación. Estos valores quedan solo como respaldo legacy y defensa ante loops.</div>';
        echo '<div class="commercial-inline-checks">';
