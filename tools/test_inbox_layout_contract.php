...
$assertions = array(
    'La cabecera principal distribuye sus controles en tres zonas' => strpos($inbox, '.inbox-topbar{display:grid;grid-template-columns:auto minmax(0,1fr) auto;align-items:center;gap:10px;padding:8px 16px;background:#075e54;border-bottom:1px solid rgba(0,0,0,.15);flex-shrink:0;min-height:50px;overflow-x:auto;scrollbar-width:none}') !== false && strpos($inbox, '.inbox-topbar .inbox-toggles{justify-self:center}') !== false && strpos($inbox, 'class="inbox-topbar__right"') !== false,
    ...
    'La agenda usa un icono de teléfono' => strpos($inbox, 'id="inboxAgendaBtn"') !== false && strpos($inbox, '☎') !== false && strpos($inbox, '☎ Agenda Comercial') !== false,
